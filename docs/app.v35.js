...
(function () {
  ...
  const Auth = (() => {
    ...
    const setMode = (mode) => {
      ...
      e.title.textContent = isReg ? 'Create Account' : 'Login';
      if(e.loginForm) e.loginForm.classList.toggle('hidden', isReg);
      if(e.regForm) e.regForm.classList.toggle('hidden', !isReg);
      if(e.loginError) e.loginError.textContent = '';
      if(e.regError) e.regError.textContent = '';
    };

    const setUserPill = () => {
      const e = els();
      const logged = !!state.ui.auth.loggedIn;
      if(e.userPill) e.userPill.classList.toggle('hidden', !logged);
      if(logged && e.userEmail) e.userEmail.textContent = state.ui.auth.email || '';
    };
    ...
    const bind = () => {
      ...
      if(e.goRegister) e.goRegister.onclick = (ev) => { ev.preventDefault(); setMode('register'); };
      if(e.goLogin) e.goLogin.onclick = (ev) => { ev.preventDefault(); setMode('login'); };

      if(e.loginForm) e.loginForm.onsubmit = async (ev) => {
        ev.preventDefault();
        try{
          await login((e.loginEmail.value||'').trim(), e.loginPass.value||'');
        }catch(err){
          if(e.loginError) e.loginError.textContent = err.message || String(err);
        }
      };

      if(e.regForm) e.regForm.onsubmit = async (ev) => {
        ev.preventDefault();
        try{
          await register(
            (e.regName.value||'').trim(),
            (e.regEmail.value||'').trim(),
            e.regPass.value||'',
            e.regPass2.value||'',
            !!(e.regTerms && e.regTerms.checked)
          );
        }catch(err){
          if(e.regError) e.regError.textContent = err.message || String(err);
        }
      };

      if(e.logoutBtn) e.logoutBtn.onclick = () => {
