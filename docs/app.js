...
(function () {
  ...
  const Timers = (() => {
    ...
    function drawLineChart(canvas, values){
      // Resize canvas to its displayed size (keeps chart crisp)
      const dpr = Math.max(1, window.devicePixelRatio || 1);
      const rect = canvas.getBoundingClientRect();
      const targetW = Math.max(320, Math.floor(rect.width || 0));
      const targetH = Math.max(180, Math.floor(rect.height || 0));
      const pxW = Math.floor(targetW * dpr);
      const pxH = Math.floor(targetH * dpr);
