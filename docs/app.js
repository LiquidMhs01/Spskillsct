...
(function () {
  ...
  const Timers = (() => {
    ...
    const createBtn = $("#createTimerBtn");

    const modal = $("#timerCreateModal");
    const closeBg = $("#timerCreateClose");
    const closeX = $("#timerCreateX");
    const nameInput = $("#timerNameInput");
    const saveBtn = $("#timerCreateSave");

    const cdWrap = $("#countdownSettings");
    const cdH = $("#cdHours");
    const cdM = $("#cdMinutes");
    const cdS = $("#cdSeconds");
    ...
    function openModal(){
      ...
      nameInput.value="";
      cdH.value=0; cdM.value=5; cdS.value=0;
      document.querySelector('input[name="timerKind"][value="stopwatch"]').checked=true;
    ...
    document.querySelectorAll('input[name="timerKind"]').forEach(r=>{
      r.addEventListener("change", ()=>{
        ...
        cdWrap.style.display = (kind==="countdown") ? "block" : "none";
      });
    });

    function createTimer(kind, name, targetMs){
    ...
    saveBtn.addEventListener("click", ()=>{
      ...
      if(kind==="countdown"){
        const h=parseInt(cdH.value||"0",10)||0;
        const m=parseInt(cdM.value||"0",10)||0;
        const s=parseInt(cdS.value||"0",10)||0;
        const total=(h*3600+m*60+s)*1000;
        if(total<=0){ alert("Defina uma duração maior que zero."); return; }
        createTimer("countdown", nm, total);
      } else {
        createTimer("stopwatch", nm, 0);
      }

      saveState().then(()=>{ closeModal(); render(); });
    });

    function elapsedNow(t){
      if(!t.running) return t.elapsedMs;
      return t.elapsedMs + Math.max(0, performance.now() - t.startedAt);
    }
    function remaining(t){
      return Math.max(0, (t.targetMs||0) - elapsedNow(t));
    }
    ...
    // UI checkbox synced on each render

    // DOM cache per timerId
    const dom = new Map(); // id -> {timeEl,statusEl,progEl,attemptsEl,noteEl,playBtn,chartPanel,chartCanvas,chartEmpty,toggleBtn}
    ...
    function buildCard(t){
      ...
      // Toggle do gráfico (por cronômetro)
      const toggle=document.createElement("button");
      toggle.className="timer-chart-toggle";
      toggle.type="button";
      toggle.innerHTML = `<span class="dot"></span><span>Gráfico</span>`;
      // update chart (only if open)
      if(t.kind==="stopwatch" && t.chartOpen) updateChart(id);
      ...
      play.onclick=()=>{
        ...
        saveState();
        // update immediately without waiting for next tick
        updateOne(t.id);
      };
      ...
      saveAttempt.onclick=()=>{
        t.logs = t.logs || [];
        t.logs.push({ ms: elapsedNow(t), at: Date.now() });
        saveState();
        updateOne(t.id);
        renderNotes(visibleTimers);
        updateChart(t.id);
      };
      ...
      // Toggle handler
      toggle.onclick = () => {
        if(t.kind==="countdown") return;
        t.chartOpen = !t.chartOpen;
        chartPanel.classList.toggle("open", !!t.chartOpen);
        toggle.classList.toggle("on", !!t.chartOpen);
        saveState();
        updateChart(t.id);
      };
    ...
    function updateOne(id){
      ...
      const el = elapsedNow(t);
      const shown = (t.kind==="countdown") ? remaining(t) : el;
      d.timeEl.textContent = fmt(shown);

      d.statusEl.textContent = t.running ? "Rodando" : "Pausado";
      d.playBtn.textContent = t.running ? "⏸" : "▶";

      d.attemptsEl.textContent = t.kind==="countdown"
        ? `Duração: ${fmt(t.targetMs).split('.')[0]}`
    ...
    function renderNotes(list){
      const timers = list || state.timers;
      ...
      for(const t of visibleTimers){
        ...
        d.noteEl.textContent = `Tendência: ${trend} • Primeira: ${fmt(first)} • Última: ${fmt(last)}`;
      }
    }

    function render(){
      if(!timerGrid) return;
      state.ui = state.ui || { hideTimerChart: false, timerFilter: 'stopwatch' };
      const filterRaw = state.ui.timerFilter || 'stopwatch';
      const filter = (filterRaw === 'countdown' || filterRaw === 'stopwatch') ? filterRaw : 'stopwatch';
      state.ui.timerFilter = filter;
      syncFilterUI();
      const visibleTimers = state.timers.filter(t => (filter === 'stopwatch' ? t.kind === 'stopwatch' : t.kind === 'countdown'));
      ...
      if(visibleTimers.length===0){
        const m=document.createElement("div");
        m.className="muted small-text";
        m.textContent="Nenhum cronômetro criado ainda. Clique em + Novo.";
        timerGrid.append(m);
        return;
      }
      ...
      // initial update
      for(const t of visibleTimers) updateOne(t.id);
      renderNotes();
    ...
    function tick(){
      for(const t of state.timers){
        // Only update running timers, but keep countdown finishing logic
        if(t.running || t.kind==="countdown") updateOne(t.id);
      }
      renderNotes();
      requestAnimationFrame(tick);
    }
    ...
    // Sub-abas: Cronômetros / Temporizadores
    ...
    function syncFilterUI(){
      const raw = (state.ui && state.ui.timerFilter) || "stopwatch";
      const f = (raw === "countdown" || raw === "stopwatch") ? raw : "stopwatch";
      if(state.ui) state.ui.timerFilter = f;
      if(filterStopBtn){
