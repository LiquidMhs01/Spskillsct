...
(function () {
  ...
  function loadUsers(){
    try{
      const raw = localStorage.getItem('cts_users_v1');
      const obj = raw ? JSON.parse(raw) : { users: [] };
      if(!obj.users || !Array.isArray(obj.users)) obj.users = [];
      return obj;
    }catch(e){
      return { users: [] };
    }
  ...
  function saveUsers(db){
    localStorage.setItem('cts_users_v1', JSON.stringify(db));
  }
const $ = (s) => document.querySelector(s);
  ...
  const DEFAULT_STATE = {
    ...
    timers: [],
    ui: { hideTimerChart: false, timerFilter: 'stopwatch', sidebarHidden: false, auth: { loggedIn: false, email: '' } },
    notes: { html: '', files: [], sheet: { rows: 12, cols: 6, data: [], styles: {} } },
  };
  ...
  function migrateState(s) {
    ...
    if (!s.ui || typeof s.ui !== "object") s.ui = structuredCloneSafe(DEFAULT_STATE.ui);
    if (typeof s.ui.hideTimerChart !== "boolean") s.ui.hideTimerChart = false;
    if (typeof s.ui.timerFilter !== "string") s.ui.timerFilter = "stopwatch";
    if (typeof s.ui.sidebarHidden !== "boolean") s.ui.sidebarHidden = false;
    if (!s.ui.auth || typeof s.ui.auth !== "object") s.ui.auth = { loggedIn: false, email: "" };
    if (typeof s.ui.auth.loggedIn !== "boolean") s.ui.auth.loggedIn = false;
    if (typeof s.ui.auth.email !== "string") s.ui.auth.email = "";

    if (!s.notes || typeof s.notes !== "object") s.notes = structuredCloneSafe(DEFAULT_STATE.notes);
    if (typeof s.notes.html !== "string") s.notes.html = "";
    if (!Array.isArray(s.notes.files)) s.notes.files = [];

    if (!s.notes.sheet || typeof s.notes.sheet !== "object") s.notes.sheet = structuredCloneSafe(DEFAULT_STATE.notes.sheet);
    const sh = s.notes.sheet;
    if (typeof sh.rows !== "number" || !isFinite(sh.rows)) sh.rows = DEFAULT_STATE.notes.sheet.rows;
    if (typeof sh.cols !== "number" || !isFinite(sh.cols)) sh.cols = DEFAULT_STATE.notes.sheet.cols;
    if (!Array.isArray(sh.data)) sh.data = [];
    if (!sh.styles || typeof sh.styles !== "object") sh.styles = {};
  ...
  const tabMeta = {
    ...
    notes: { title: "Anotações", subtitle: "Texto livre, anexos e planilha simples." },
  };
  ...
  function setTab(tab) {
    ...
    const tabEl = $(`#tab-${tab}`);
      ...
      else console.warn("Tab não encontrada:", tab);
  ...
  const Schedule = (() => {
    ...
    async function renderPdfPage(pageNum, zoom) {
      ...
      if (pdfPageEl) pdfPageEl.textContent = String(pdfPage);
      if (pdfPagesEl) pdfPagesEl.textContent = String(pdfDoc.numPages);
      if (pdfPager) pdfPager.classList.remove("hidden");
      applyView();
    }
  ...
  const Tasks = (() => {
    ...
    function renderLabelsChecklist(task) {
      ...
      const cnt = document.getElementById("notesFilesCount");
      if (cnt) cnt.textContent = files.length ? (files.length + " arquivo(s)") : "";
  ...
  const Notes = (() => {
    ...
    const ensureSheetData = () => {
      const sheet = state.notes.sheet;
      const rows = Math.max(1, sheet.rows|0);
      const cols = Math.max(1, sheet.cols|0);
      sheet.rows = rows;
      sheet.cols = cols;
      // initialize data
      if (!Array.isArray(sheet.data)) sheet.data = [];
      while (sheet.data.length < rows) sheet.data.push(Array(cols).fill(""));
      for (let r = 0; r < rows; r++) {
        if (!Array.isArray(sheet.data[r])) sheet.data[r] = Array(cols).fill("");
        while (sheet.data[r].length < cols) sheet.data[r].push("");
        if (sheet.data[r].length > cols) sheet.data[r] = sheet.data[r].slice(0, cols);
      }
      if (sheet.data.length > rows) sheet.data = sheet.data.slice(0, rows);
      if (!sheet.styles || typeof sheet.styles !== "object") sheet.styles = {};
    };

    const keyFor = (r,c) => `${r},${c}`;

    const renderFiles = () => {
      const list = document.getElementById("notesFilesList");
      if (!list) return;
      const files = state.notes.files || [];
      list.innerHTML = "";
      const cnt = document.getElementById("notesFilesCount");
      if (cnt) cnt.textContent = files.length ? (files.length + " arquivo(s)") : "";
      if (!files.length) {
        list.innerHTML = `<div class="muted">Nenhum anexo ainda.</div>`;
        return;
      }
      for (const f of files) {
        ...
        row.innerHTML = `
          <div class="attach-meta">
            <div class="attach-name">📎 ${escapeHtml(f.name || "arquivo")}</div>
            <div class="attach-sub">${Math.round((f.size||0)/1024)} KB • ${escapeHtml(f.type||"")}</div>
          </div>
          <div class="attach-actions">
            <a class="btn small" download="${escapeHtml(f.name||"arquivo")}" href="${f.dataUrl}">Baixar</a>
            <button class="btn danger small" data-del="${f.id}">Remover</button>
          </div>`;
        row.querySelector('[data-del]').onclick = () => {
          state.notes.files = (state.notes.files||[]).filter(x => x.id !== f.id);
          saveState();
          renderFiles();
        };
    ...
    const renderSheet = () => {
      ensureSheetData();
      const table = document.getElementById("notesSheet");
      if (!table) return;
      const sheet = state.notes.sheet;
      table.innerHTML = "";
      // header
      ...
      for (let c=0;c<sheet.cols;c++){
        const th=document.createElement("th");
        th.textContent=String.fromCharCode(65+(c%26)) + (c>=26? Math.floor(c/26):"");
        hr.appendChild(th);
      }
      ...
      for (let r=0;r<sheet.rows;r++){
        ...
        for (let c=0;c<sheet.cols;c++){
          ...
          const st = sheet.styles[keyFor(r,c)];
          if (st){
            if (st.bold) td.style.fontWeight="900";
            if (st.bg) td.style.background=st.bg;
            if (st.align) td.style.textAlign=st.align;
          }
          td.onfocus = () => selectCell(td);
          td.onclick = () => selectCell(td);
          td.oninput = () => {
            sheet.data[r][c] = td.textContent || "";
            saveState();
          };
          tr.appendChild(td);
        }
    ...
    const clearSelected = () => {
      document.querySelectorAll("#notesSheet td.selected").forEach(el => el.classList.remove("selected"));
    };
    ...
    const applyCellStyle = (patch) => {
      if (!selectedCell) return;
      const r = parseInt(selectedCell.dataset.r,10);
      const c = parseInt(selectedCell.dataset.c,10);
      const sheet = state.notes.sheet;
      const k = keyFor(r,c);
      const cur = sheet.styles[k] || {};
      sheet.styles[k] = {...cur, ...patch};
      saveState();
      renderSheet();
      // reselect
      const td = document.querySelector(`#notesSheet td[data-r="${r}"][data-c="${c}"]`);
      if (td) selectCell(td);
    };
    ...
    const bindToolbar = () => {
      ...
      document.querySelectorAll(".notes-toolbar [data-cmd]").forEach(btn => {
        btn.onclick = () => {
          ...
          editor.focus();
          if (cmd === "formatBlock") document.execCommand(cmd, false, arg || "P");
          else document.execCommand(cmd, false, null);
          state.notes.html = editor.innerHTML;
          saveState();
        };
      });
      ...
      // attachments
      const filesInput = document.getElementById("notesFiles");

      const addBtn = document.getElementById("notesAddFileBtn");
      if (addBtn && filesInput) addBtn.onclick = () => filesInput.click();
      const cnt = document.getElementById("notesFilesCount");

      if (filesInput) filesInput.onchange = async () => {
        const files = Array.from(filesInput.files || []);
        for (const f of files) {
          const dataUrl = await fileToDataUrl(f);
          state.notes.files.push({ id: crypto.randomUUID(), name: f.name, type: f.type, size: f.size, dataUrl });
        }
        filesInput.value = "";
        saveState();
        renderFiles();
      ...
      const rmAll = document.getElementById("notesRemoveAll");
      if (rmAll) rmAll.onclick = () => {
        if (!confirm("Remover todos os anexos?")) return;
        state.notes.files = [];
        saveState();
        renderFiles();
      };
      ...
      // sheet buttons
      const addRow = document.getElementById("sheetAddRow");
      const addCol = document.getElementById("sheetAddCol");
      const delRow = document.getElementById("sheetDelRow");
      const delCol = document.getElementById("sheetDelCol");
      const clearSheet = document.getElementById("sheetClear");
      if (addRow) addRow.onclick = () => { state.notes.sheet.rows += 1; saveState(); renderSheet(); };
      if (addCol) addCol.onclick = () => { state.notes.sheet.cols += 1; saveState(); renderSheet(); };
      if (delRow) delRow.onclick = () => { state.notes.sheet.rows = Math.max(1, state.notes.sheet.rows-1); saveState(); renderSheet(); };
      if (delCol) delCol.onclick = () => { state.notes.sheet.cols = Math.max(1, state.notes.sheet.cols-1); saveState(); renderSheet(); };
      if (clearSheet) clearSheet.onclick = () => {
        if(!confirm("Limpar toda a planilha?")) return;
        state.notes.sheet.data = [];
        state.notes.sheet.styles = {};
        saveState();
        renderSheet();
      };

      const cellBold = document.getElementById("cellBold");
      const alL = document.getElementById("cellAlignLeft");
      const alC = document.getElementById("cellAlignCenter");
      const alR = document.getElementById("cellAlignRight");
      const bg = document.getElementById("cellBg");

      if (cellBold) cellBold.onclick = () => {
        if(!selectedCell) return;
        const r=parseInt(selectedCell.dataset.r,10), c=parseInt(selectedCell.dataset.c,10);
        const k=keyFor(r,c);
        const cur = state.notes.sheet.styles[k] || {};
        applyCellStyle({ bold: !cur.bold });
      };
      if (alL) alL.onclick = () => applyCellStyle({ align: "left" });
      if (alC) alC.onclick = () => applyCellStyle({ align: "center" });
      if (alR) alR.onclick = () => applyCellStyle({ align: "right" });
      if (bg) bg.oninput = () => applyCellStyle({ bg: bg.value });
    };
    ...
    const render = () => {
      const editor = document.getElementById("notesEditor");
      if (editor) editor.innerHTML = state.notes.html || "";
      renderFiles();
      renderSheet();
      bindToolbar();
    };
...
// ---------- Export/Import/Reset ----------
  ...
  const Auth = (() => {
    ...
    const setMode = (mode) => {
      ...
      const regError = el('registerError');
      const isReg = mode === 'register';
      if(title) title.textContent = isReg ? 'Create Account' : 'Login';
      if(loginForm) loginForm.classList.toggle('hidden', isReg);
      if(regForm) regForm.classList.toggle('hidden', !isReg);
    ...
    const setUserPill = () => {
      ...
      const logged = !!(state?.ui?.auth?.loggedIn);
      if(pill) pill.classList.toggle('hidden', !logged);
      if(logged && emailEl) emailEl.textContent = state.ui.auth.email || '';
    };
    ...
    const login = async (email, pass) => {
      ...
      if(u.passHash !== h) throw new Error('Email ou senha inválidos.');
      state.ui.auth.loggedIn = true;
      state.ui.auth.email = u.email;
      saveState();
      setUserPill();
      hide();
    };

    const register = async (name, email, pass, pass2, terms) => {
      if(!terms) throw new Error('Você precisa aceitar os termos.');
      ...
      const db = loadUsers();
      const exists = db.users.some(x => (x.email||'').toLowerCase() === (email||'').toLowerCase());
      if(exists) throw new Error('Este email já está cadastrado.');
      const h = await sha256Hex(pass);
      db.users.push({ id: crypto.randomUUID(), name, email, passHash: h, createdAt: Date.now() });
      saveUsers(db);
      state.ui.auth.loggedIn = true;
      state.ui.auth.email = email;
      saveState();
      setUserPill();
      hide();
    };
    ...
    const bind = () => {
      ...
      const goLogin = el('goLogin');
      if(goRegister) goRegister.onclick = (ev) => { ev.preventDefault(); setMode('register'); };
      if(goLogin) goLogin.onclick = (ev) => { ev.preventDefault(); setMode('login'); };
      ...
      if(loginForm) loginForm.onsubmit = async (ev) => {
        ...
        catch(err){ const le = el('loginError'); if(le) le.textContent = err.message || String(err); }
      };

      if(regForm) regForm.onsubmit = async (ev) => {
        ev.preventDefault();
        try{
          await register(
            ...
            el('regPass')?.value || '',
            el('regPass2')?.value || '',
            !!(el('regTerms') && el('regTerms').checked)
          );
        }catch(err){
          const re = el('registerError'); if(re) re.textContent = err.message || String(err);
        }
      };
    ...
    const requireLogin = () => {
      if(!(state?.ui?.auth?.loggedIn)) show('login'); else hide();
      setUserPill();
    };

    return { bind, requireLogin, show, hide, setMode, setUserPill };
  })();
...
function boot() {
    ...
    db.app
      ...
      .then(() => {
        ...
        const btnShow = document.getElementById("sidebarShow");
        const applySidebar = () => {
          const hidden = !!(state.ui && state.ui.sidebarHidden);
          if (appRoot) appRoot.classList.toggle("sidebar-hidden", hidden);
          if (btnShow) btnShow.classList.toggle("hidden", !hidden);
          if (btnHide) btnHide.textContent = hidden ? "❯" : "❮";
        };
        const toggleSidebar = (force) => {
          const next = typeof force === "boolean" ? force : !(state.ui && state.ui.sidebarHidden);
          state.ui.sidebarHidden = next;
          saveState();
          applySidebar();
        };
        if (btnHide) btnHide.addEventListener("click", () => toggleSidebar());
        if (btnShow) btnShow.addEventListener("click", () => toggleSidebar(false));
        applySidebar();

        // schedule picker button + filename
        const pickBtn = document.getElementById("pickScheduleBtn");
        const schedInput = document.getElementById("scheduleFile");
        const schedName = document.getElementById("scheduleFileName");
        if (pickBtn && schedInput) pickBtn.addEventListener("click", () => schedInput.click());
        if (schedInput) {
          schedInput.addEventListener("change", () => {
            const f = schedInput.files && schedInput.files[0];
            if (schedName) schedName.textContent = f ? f.name : "Nenhum arquivo selecionado";
          });
        }
